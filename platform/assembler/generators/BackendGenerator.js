...
class BackendGenerator {
  ...
  /**
   * Plan H follow-up migration: drops column-level NOT NULL on every required
   * reference field that belongs to an entity with a "Draft" status option.
   *
   * The auto-draft flow (POST /<slug>/draft fired on /new mount in the
   * generated form page) inserts a row BEFORE the user has filled any
   * reference. The service-layer validator already keeps these fields
   * required for non-Draft statuses, so the column-level NOT NULL was
   * pure friction — it threw "null value in column ... violates not-null
   * constraint" the moment the form page tried to auto-draft.
   *
   * Newly generated databases get this relaxed in 001_initial_schema.sql via
   * `_shouldEmitNotNull()` in schemaGenerator. This 003 migration is for
   * pre-existing deployments whose 001 was emitted before that fix landed —
   * it ALTERs the existing tables in place. Both paths use IS-NULLABLE guards
   * so re-runs are safe.
   */
  _buildRelaxDraftFkSql(entities = []) {
    ...
    for (const entity of list) {
      ...
      if (!opts.some((o) => String(o || '').trim().toLowerCase() === 'draft')) continue;
      for (const field of fields) {
        if (!field || !field.required) continue;
        if (String(field.type || '').toLowerCase() !== 'reference') continue;
        targets.push({ slug: String(entity.slug), column: String(field.name) });
      }
    }
    ...
    const header = [
      ...
      '--',
      '-- Plan H follow-up: drop NOT NULL on FK columns that the auto-draft',
      '-- (POST /<slug>/draft) flow cannot pre-fill. The service validator',
      '-- still enforces required-ness for non-Draft statuses, so this only',
      '-- affects rows that are still in `status = \'Draft\'`.',
      '',
    ];

    const blocks = targets.map(({ slug, column }) => {
      ...
      return [
        ...
        `      AND table_name = '${safeSlug}'`,
        `      AND column_name = '${safeCol}'`,
        "      AND is_nullable = 'NO'",
        '  ) THEN',
        `    ALTER TABLE "${safeSlug}" ALTER COLUMN "${safeCol}" DROP NOT NULL;`,
        '  END IF;',
      ].join('\n');
    });
