...
${authClose}    </ToastProvider>
  ...
  async _generateSharedComponents(outputDir) {
    ...
    await this.brickRepo.copyFile('frontend-bricks/layouts/Topbar.tsx', path.join(outputDir, 'src/components/layout/Topbar.tsx'));
  }
  ...
  async generateDashboardHome(outputDir, entities, sdf = {}) {
    ...
    const dashboardHome = buildDashboardHome({ lowStockCfg, expiryCfg, activityCfg, enableReportsPage: scheduledReportsCfg.enabled === true });
  ...
  async generateSidebar(outputDir, entities, sdf = {}) {
    ...
    const sidebarContent = buildSidebar({
      visibleEntities,
      capitalize: (s) => this._capitalize(s),
      formatLabel: (s) => this._formatLabel(s),
      getModuleKey: (e) => this._getModuleKey(e),
      enableActivityLog,
      enableReports,
      workflowBadges,
