// Plan C — wizard wiring (inventory.v4)
//
// No new questions vs inventory.v3; the version bump aligns with hr.v3 and
// invoice.v3. Cross-pack inventory questions live in invoice.v3 (invoice_stock_link)
// and reservations stays here unchanged.
const INVENTORY_V4_QUESTIONS = [
  {
  ...
  },
  {
    ...
    prompt: 'Hold products aside for an order before you finalize/ship it?',
    type: 'yes_no',
    required: true,
  ...
  },
  {
    ...
    prompt: 'Order from suppliers and want to compare what you ordered to what actually arrived?',
    type: 'yes_no',
    required: true,
    section: 'Inventory Capabilities',
    question_number: 4,
  ...
  },
  {
    ...
    prompt: 'Want to do periodic counts to make sure what is on the shelf matches the system?',
    type: 'yes_no',
    required: true,
    section: 'Inventory Capabilities',
    question_number: 5,
  ...
  },
  {
    ...
    prompt: 'Do you need batch or lot numbers on products? (common for food, medicine, chemicals)',
    type: 'choice',
...
module.exports = {
  module: 'inventory',
  version: 'inventory.v4',
  template_type: 'sdf_impact_only',
  source_path: null,
  getQuestions() {
    return JSON.parse(JSON.stringify(INVENTORY_V4_QUESTIONS));
  },
};
