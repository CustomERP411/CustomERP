 ...
 *
 * Drop this file into the ROOT of any STANDALONE-PACKAGED CustomERP
 * project — the directory that contains `app/`, `data/`, `runtime/`
 * and `start.bat`/`start.command`/`start.sh` — and run it from there.
 *
 *   node seed-mock-data.js [--reset] [--volume small|medium|large]
 *                          [--seed N] [--only s,s,...] [--skip s,s,...]
 *                          [--dry-run]
 *
 * (The dev-mode equivalent that talks to PostgreSQL lives in
 *  scripts/seed-mock-data.template.js and is intended for the generated
 *  ERPs that ship with `backend/`, `frontend/`, `dev.ps1` and
 *  `docker-compose.yml`. Same CLI, same playbooks, different DB driver.)
 *
 * What it does
 *   - Reads `app/src/systemConfig.js` (always emitted by the assembler)
 *     to discover entity slugs, module configs, audit-log allowlist,
 *     scheduled-report config, RBAC group definitions, etc.
 *   - Optionally reads `sdf.json` from the bundle root for richer
 *     per-field hints (options, references, computed flags).
 *   - Opens the bundled SQLite database at `data/erp.db` (or whatever
 *     `SQLITE_PATH` env var points to, mirroring `app/src/repository/
 *     sqliteDb.js`) using `better-sqlite3` from `app/node_modules`.
 *   - For every entity slug it walks `PRAGMA table_info(...)` to learn
 *     the live shape, generates plausible mock values per column
 ...
 *
 * Idempotency
 *   - `--reset` clears every non-system business table first.
 *   - Without `--reset` the script skips any table that is already
 *     non-empty.
 *
 * Dependencies
 *   - `better-sqlite3` is required and is loaded from
 *     `app/node_modules` (always installed by the standalone packager).
 *   - `bcryptjs` is optional — if present it is used to hash demo user
 *     passwords; if absent the script falls back to a SHA-256 hash, the
 *     same fallback `app/src/rbac/rbacSeed.js` uses.
 *   - Should the bundle have been moved/stripped of `app/node_modules`,
 *     the script auto-installs `better-sqlite3` into a sibling
 *     `.seed-mock-data-deps/` folder on first run.
 */
...
const SYSCFG_PATH = path.join(APP_SRC_DIR, 'systemConfig.js');
const DATA_DIR = path.join(ROOT, 'data');
const DEFAULT_DB_PATH = path.join(DATA_DIR, 'erp.db');

if (!fs.existsSync(APP_DIR) || !fs.existsSync(SYSCFG_PATH)) {
  console.error('[seed-mock-data] Could not find a packaged CustomERP bundle in the current directory.');
  console.error(`                 CWD: ${ROOT}`);
  console.error('');
  console.error('                 This SQLite seeder must live next to app/, data/, runtime/');
  console.error('                 and start.bat/start.command/start.sh — i.e. the ROOT of an');
  console.error('                 ERP packaged with the standalone packager.');
  console.error('');
  console.error('                 Specifically it expects:');
  console.error(`                   - ${path.join('app', 'src', 'systemConfig.js')}   (assembler-emitted config)`);
  console.error(`                   - ${path.join('data', 'erp.db')}                (SQLite database; created on first start.bat run)`);
  console.error('');
  if (fs.existsSync(path.join(ROOT, 'backend')) && fs.existsSync(path.join(ROOT, 'docker-compose.yml'))) {
    console.error('                 The current folder looks like a DEV-MODE generated ERP');
    console.error('                 (it has backend/ and docker-compose.yml). For that flavor');
    console.error('                 use the Postgres seeder instead:');
    console.error('                   node seed-mock-data.js   (the dev-mode template)');
    console.error('');
  }
  // If the user ran from the CustomERP platform repo root, list the
  // packaged bundles they could cd into.
  ...
  if (fs.existsSync(platformDir) && fs.existsSync(generatedDir)) {
    try {
      const candidates = fs
        ...
        .filter((d) =>
          d.isDirectory() &&
          fs.existsSync(path.join(generatedDir, d.name, 'app', 'src', 'systemConfig.js'))
        )
        .map((d) => d.name);
      if (candidates.length) {
        console.error('                 You appear to be at the CustomERP platform root. Available');
        console.error('                 packaged bundles:');
        for (const name of candidates) console.error(`                   - generated${path.sep}${name}`);
        console.error('');
        console.error('                 cd into one of those and run this seeder from there.');
      }
    } catch {
...
// Resolve runtime deps from one of:
//   1. <bundle>/app/node_modules                 (packaged backend deps — always present
//      in a freshly-extracted bundle, since the standalone packager runs `npm install`
//      inside app/)
//   2. <bundle>/.seed-mock-data-deps/node_modules (auto-installed by this script if
//      app/node_modules is missing — e.g. a partial/zipped-up bundle)
//   3. The current Node module resolution         (last-resort fallback)
//
// We require `better-sqlite3`. `bcryptjs` is optional — present only in dev-mode
// generated ERPs; the standalone bundle's RBAC seed itself falls back to SHA-256.

const HOST_APP_NM = path.join(APP_DIR, 'node_modules');
...
function loadDep(name) {
  return (
    tryRequireFrom(HOST_APP_NM, name) ||
...
function ensureFallbackDeps() {
  ...
  const missing = required.filter((d) => !tryRequireFrom(HOST_APP_NM, d) && !tryRequireFrom(FALLBACK_NM, d) && !(function () { try { require(d); return true; } catch { return false; } })());
  if (missing.length === 0) return;
  console.log(`[seed-mock-data] Installing helper dependencies (${missing.join(', ')}) into ${FALLBACK_DEPS_DIR} ...`);
  console.log('                 (this happens once; needs npm + a C/C++ toolchain because');
  console.log('                  better-sqlite3 has a native module).');
  ...
  try {
    ...
    console.error(`[seed-mock-data] npm install failed: ${e.message}.`);
    console.error('                 The packaged bundle should already ship better-sqlite3 in');
    console.error(`                 ${HOST_APP_NM}.`);
    console.error('                 If that folder is missing, re-extract the bundle ZIP from scratch');
    console.error('                 (do not copy individual files out of it).');
    process.exit(2);
  }
}
...
const Database = loadDep('better-sqlite3');
if (!Database) {
  console.error('[seed-mock-data] better-sqlite3 could not be loaded.');
  console.error(`                 Looked in: ${HOST_APP_NM}`);
  console.error(`                            ${FALLBACK_NM}`);
  process.exit(2);
}

// bcryptjs is optional. Mirror app/src/rbac/rbacSeed.js — fall back to SHA-256
// so seeded user passwords match what the running ERP would compute.
const bcryptMod = loadDep('bcryptjs');
const cryptoMod = require('crypto');
const bcrypt = bcryptMod || {
  hashSync: (plain) => cryptoMod.createHash('sha256').update(String(plain)).digest('hex'),
};
...
const systemConfig = require(SYSCFG_PATH);
...
// Lifecycle / approval status labels.
//
// The assembler propagates configurable status arrays into systemConfig
// (e.g. invoice_priority_a.lifecycle.statuses, hr_priority_a.leave_approvals.statuses)
// so localized ERPs can use Turkish/German/etc. labels. We map them
// position-by-position to the seeder's logical concepts so that filters
// in the live UI match what we insert.
//
// Logical positions for invoice lifecycle: [Draft, Sent, Paid, Overdue, Cancelled]
// Logical positions for leave approvals:   [Pending, Approved, Rejected, Cancelled]
function pickStatuses(configured, defaults) {
  if (!Array.isArray(configured) || configured.length < defaults.length) return defaults.slice();
  return defaults.map((_, i) => String(configured[i] || defaults[i]));
}
const INV_LIFECYCLE = pickStatuses(
  invc.lifecycle && invc.lifecycle.statuses,
  ['Draft', 'Sent', 'Paid', 'Overdue', 'Cancelled']
);
const [INV_DRAFT, INV_SENT, INV_PAID, INV_OVERDUE, INV_CANCELLED] = INV_LIFECYCLE;

const LEAVE_LIFECYCLE = pickStatuses(
  hr.leave_approvals && hr.leave_approvals.statuses,
  ['Pending', 'Approved', 'Rejected', 'Cancelled']
);
const [LV_PENDING, LV_APPROVED, LV_REJECTED, LV_CANCELLED] = LEAVE_LIFECYCLE;

// ─────────────────────────────────────────────────────────────────────
// 4. SQLite connect (mirrors app/src/repository/sqliteDb.js) + a tiny
//    "pg pool"-shaped shim that translates PG SQL on the fly so the
//    rest of this script can stay identical to the dev-mode seeder.
// ─────────────────────────────────────────────────────────────────────

const SQLITE_PATH = process.env.SQLITE_PATH || DEFAULT_DB_PATH;

if (!fs.existsSync(SQLITE_PATH)) {
  // The app creates the DB on first boot. If the user runs the seeder
  // before ever launching the ERP, the file will be missing.
  console.error(`[seed-mock-data] SQLite database not found at: ${SQLITE_PATH}`);
  console.error('                 Start the ERP at least once first so it can create the DB');
  console.error('                 and run schema migrations:');
  console.error('                   - Windows : double-click start.bat');
  console.error('                   - macOS   : double-click start.command (or run ./start.sh)');
  console.error('                   - Linux   : ./start.sh');
  console.error('                 You can stop it after it prints the listening URL, then');
  console.error('                 re-run this seeder.');
  process.exit(1);
}

fs.mkdirSync(path.dirname(SQLITE_PATH), { recursive: true });
const _db = new Database(SQLITE_PATH);
_db.pragma('journal_mode = WAL');
_db.pragma('foreign_keys = OFF'); // matches mass-insert workflows; assembler uses ON at runtime
_db.pragma('busy_timeout = 5000');

// Translate a PG statement into a SQLite-friendly one and a positional
// parameter list. We only need to handle the small surface area this
// seeder actually uses: $N placeholders, ::int casts, RETURNING *,
// information_schema.columns/tables introspection, COUNT(*) and
// TRUNCATE … RESTART IDENTITY CASCADE.
function translateSqlForSqlite(text, values) {
  let sql = String(text);
  const params = Array.isArray(values) ? values.slice() : [];

  // PG type casts that SQLite doesn't speak — strip ::int / ::text / etc.
  sql = sql.replace(/::\s*[a-zA-Z_][a-zA-Z0-9_]*/g, '');

  // information_schema.columns → PRAGMA table_info(<bound name>)
  // The seeder only ever uses one shape:
  ...
  // Used in reset(): SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE'
  if (/information_schema\.tables/i.test(sql)) {
    return { kind: 'tables' };
  }

  // TRUNCATE list RESTART IDENTITY CASCADE → DELETE FROM each table
  const truncMatch = sql.match(/^\s*TRUNCATE\s+(.+?)\s+RESTART\s+IDENTITY\s+CASCADE\s*;?\s*$/i);
  if (truncMatch) {
    const list = truncMatch[1]
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
  ...
  // $1, $2, ... → ?, ?, ... (better-sqlite3 uses ? positional)
  // Replace in numeric order so we keep the param array aligned.
  // We rewrite via a regex pass that ignores any literal '$' inside string
  // literals (this seeder doesn't have any).
  sql = sql.replace(/\$\d+/g, '?');

  // RETURNING * is supported natively in SQLite >= 3.35 (better-sqlite3
  // bundles a newer SQLite), so no rewriting needed.

  return { kind: 'sql', sql, params };
}

const COL_PRAGMA_CACHE = new Map();

function pgIsNullableFromPragma(pragmaRow) {
  // PRAGMA table_info: notnull = 1 → NOT NULL, 0 → nullable
  return Number(pragmaRow.notnull) === 0 ? 'YES' : 'NO';
}

function pgDataTypeFromPragma(pragmaRow) {
  const raw = String(pragmaRow.type || '').toUpperCase();
  if (!raw) return 'text';
...
const pool = {
  async query(text, values) {
    ...
    if (plan.kind === 'columns') {
      ...
      if (COL_PRAGMA_CACHE.has(slug)) return { rows: COL_PRAGMA_CACHE.get(slug) };
      ...
      COL_PRAGMA_CACHE.set(slug, mapped);
      return { rows: mapped };
    }
    ...
    if (plan.kind === 'truncate') {
      // DELETE rather than DROP — the schema stays intact.
      // Inside one transaction so a partial failure rolls back.
      const stmts = plan.tables.map((t) => _db.prepare(`DELETE FROM ${t}`));
      const seqStmts = plan.tables.map((t) => {
        const bare = t.replace(/^"(.*)"$/, '$1');
        return _db.prepare(`DELETE FROM sqlite_sequence WHERE name = '${bare.replace(/'/g, "''")}'`);
      });
      const tx = _db.transaction(() => {
        for (const s of stmts) s.run();
        for (const s of seqStmts) {
          try { s.run(); } catch { /* sqlite_sequence may not exist if no AUTOINCREMENT */ }
        }
      });
      tx();
      return { rows: [] };
    ...
    // Plain SQL path
    // better-sqlite3 only accepts null|number|bigint|string|Buffer for
    // bound params. Normalise everything else (the seeder's coerce()
    // already does most of this, but a few callsites bind raw values).
    const safeParams = plan.params.map((v) => {
    ...
    const trimmed = plan.sql.trim().toLowerCase();
    const stmt = _db.prepare(plan.sql);
    if (trimmed.startsWith('select') || / returning /i.test(plan.sql)) {
...
async function seedGeneric(slug, count, label) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    try {
      ...
      if (/duplicate key|null value|violates|constraint failed/i.test(e.message)) continue;
      throw e;
    }
  }
...
async function seedProcurement() {
  ...
  for (let i = 0; i < totalOrders; i++) {
    // Some SDFs model suppliers as a separate entity (FK), others bake
    // the supplier name into the PO row directly. Cover both: if a
    // vendor cache is available use the FK, otherwise just generate a
    // company name. We always send both fields and insertRow drops
    // whichever isn't in the actual schema.
    const vendor = pickFromCache(SLUGS.vendor);
    const supplierName = vendor && vendor.name ? vendor.name : makeCompanyName();
    ...
    const order = await insertRow(orderSlug, {
      ...
      note: 'Auto-generated procurement order.',
    });
...
async function seedArrivals() {
  ...
  for (const order of (cache[SLUGS.po] || []).slice(0, 12)) {
    ...
    const receivedAt = isoDt(daysAgo(intBetween(0, 60)));
    const docket = await insertRow(grnSlug, {
      ...
      status,
      received_by: receiver ? receiver.id : null,
      ...
      note: 'Auto-generated arrival docket.',
    });
...
async function seedBilling() {
  ...
  for (let i = 0; i < V.invoices; i++) {
    const client = pickFromCache(SLUGS.client);
    if (!client) break;
    const status = pickOneWeighted([[INV_DRAFT, 15], [INV_SENT, 25], [INV_PAID, 35], [INV_OVERDUE, 15], [INV_CANCELLED, 10]]);
    ...
    const paid = status === INV_PAID ? grand : status === INV_OVERDUE ? Number((grand * floatBetween(0, 0.6)).toFixed(2)) : 0;
    ...
    const inv = await insertRow(slug, {
      ...
      posted_at: status === INV_DRAFT ? null : isoDt(issueDate),
      cancelled_at: status === INV_CANCELLED ? isoDt(daysAgo(intBetween(0, 30))) : null,
      currency: 'USD',
...
async function seedSettlements() {
  ...
  for (let i = 0; i < paidInvoices.length; i++) {
    ...
    const status = invRow.status === INV_PAID ? 'Posted' : pickOne(['Posted', 'Draft']);
    ...
    if (allocSlug && (await tableExists(allocSlug))) {
      const allocRow = await insertRow(allocSlug, {
        ...
        amount,
        allocated_amount: amount,
        allocated_at: isoDt(daysAgo(intBetween(0, 60))),
...
async function seedTimeOff() {
  ...
  for (let i = 0; i < Math.min(employees.length * 2, employees.length + 15); i++) {
    ...
    const status = pickOneWeighted([[LV_PENDING, 20], [LV_APPROVED, 50], [LV_REJECTED, 15], [LV_CANCELLED, 15]]);
    const approver = status === LV_APPROVED || status === LV_REJECTED
    ...
    const row = await insertRow(slug, {
      ...
      approved_at: status === LV_APPROVED ? isoDt(daysAgo(intBetween(0, 90))) : null,
      rejected_at: status === LV_REJECTED ? isoDt(daysAgo(intBetween(0, 90))) : null,
      rejection_reason: status === LV_REJECTED ? 'Conflict with project deadlines.' : null,
      decision_key: status === LV_PENDING ? null : `DK-${uuid()}`,
      cancelled_at: status === LV_CANCELLED ? isoDt(daysAgo(intBetween(0, 60))) : null,
    });
    pushCache(slug, row);
    n += 1;
  }
...
// Resilience wrapper: run a playbook, log+swallow errors so an
// unexpected NOT NULL / unique / FK / type mismatch in one entity does
// not abort the entire seeding run. The seeder is meant to work on
// *any* SDF, including ones we haven't seen — schemas drift, fields
// get added/removed/renamed in customisation, and a single bad row
// shouldn't kill the demo.
async function safe(label, fn) {
  try {
    await fn();
  } catch (e) {
    const msg = e && e.message ? e.message : String(e);
    log(`✗ ${label} skipped: ${msg}`, 'yellow');
    if (process.env.SEED_DEBUG === '1' && e && e.stack) {
...
async function main() {
  ...
  try {
    ...
    console.error(`\n[seed-mock-data] Could not open the SQLite database.`);
    console.error(`Path: ${SQLITE_PATH}`);
    console.error(`Set SQLITE_PATH to override (default: <bundle>/data/erp.db).`);
  ...
  if (args.reset) {
    header('Reset');
    await safe('reset', reset);
  }

  header('Master data');
  await safe('sites',        seedSites);
  await safe('vendors',      seedVendors);
  await safe('clients',      () => seedClients(SLUGS.client));
  if (STUB_CUSTOMERS) await safe('customers (stub)', () => seedClients('customers'));
  await safe('departments',  seedDepartments);
  await safe('employees',    seedEmployees);
  await safe('demo users',   seedDemoUsers);

  header('Stock');
  await safe('stock articles', seedStockArticles);
  await safe('stock movements', seedMovements);
  await safe('reservations',   seedHolds);

  header('Procurement & receiving');
  await safe('purchase orders', seedProcurement);
  await safe('goods receipts',  seedArrivals);

  header('Sales / dispatch');
  await safe('dispatch orders', seedDispatch);
  await safe('sales orders (stub)', seedSalesStub);

  header('Cycle counts');
  await safe('cycle counts', seedCycleCounts);

  header('Billing');
  await safe('invoices',     seedBilling);
  await safe('payments',     seedSettlements);
  await safe('credit notes', seedAdjustments);

  header('HR');
  await safe('leave balances',  seedTimeOffBalances);
  await safe('leave requests',  seedTimeOff);
  await safe('attendance',      seedAttendance);
  await safe('shifts',          seedShifts);
  await safe('timesheets',      seedTimesheets);
  await safe('compensation ledger',    seedCompLedger);
  await safe('compensation snapshots', seedCompSnapshots);

  header('Self-reference fixups');
  await safe('self-reference fixups', fixSelfRefs);

  header('System');
  await safe('audit logs', seedAuditLogs);
  await safe('reports',    seedReports);

  header('Generic fallback (unrecognised tables)');
  await safe('generic fallback', seedRemaining);

  await safe('summary', printSummary);
