...
# Accept either the renamed copy ("seed-mock-data-sqlite.js") or the
# platform-side template name ("seed-mock-data-sqlite.template.js"),
# so users don't have to rename the file after copying it in.
$Candidates = @(
    (Join-Path $PSScriptRoot 'seed-mock-data-sqlite.js'),
    (Join-Path $PSScriptRoot 'seed-mock-data-sqlite.template.js')
)
$ScriptPath = $Candidates | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $ScriptPath) {
    Write-Host "[seed] Seeder script was not found next to this launcher." -ForegroundColor Red
